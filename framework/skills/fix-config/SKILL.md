...
Audit this project's SweetClaude setup and fix what's broken.

**This skill changes SweetClaude's own configuration, not your project code. Every change is proposed before applied.**
...
> "SweetClaude isn't set up for this project. Run `/sweetclaude:init` or `/sweetclaude:sherpa-adopt` first."
...
If phase doesn't match reality, propose the correction:
> "phase.yaml says {current} but the project is actually in {recommended} based on: {evidence}. Update?"
...
1. **Doc table / repo structure section** — does it list directories that actually exist? Does it omit directories that do exist?
   - `ls` key directories and compare against what CLAUDE.md claims
   - Flag: directories listed but missing, directories that exist but aren't listed
...
4. **Project description** — is it still accurate or has the project evolved?

For each issue found, propose a specific fix:
> "CLAUDE.md says {wrong thing}. Actually: {right thing}. Fix?"
...
> "Found {artifact} at {actual location}. SweetClaude expects it at {expected location}. Move it, symlink it, or leave it and update SweetClaude's reference?"

Don't assume — ask. The user may have good reasons for the current location.
...
> "All SweetClaude registers are empty, but the project has {N} commits and existing docs. Want me to populate them retroactively from git history and existing docs? This gives SweetClaude context about what's already been decided."
...
> "These SweetClaude files aren't tracked in git: {list}. Commit them?"
...
- **Propose, don't apply.** Every change needs user approval.
- **Don't move project files without asking.** The user may have reasons for current locations.
- **Retroactive register entries are drafts.** Mark them `[retroactive]` so the user knows they weren't captured in real-time.
- **Don't judge the project.** This skill fixes SweetClaude config, not project quality.
