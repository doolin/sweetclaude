...
Read personas from `product/discovery` output or `strategy/ideal-customer-profile.md`. Success criteria must map to specific personas and tasks.
...
2. **For each task, define success.** Each criterion must be:
   - **Observable** — you can watch someone do it or measure it
   - **Binary** — after ship, it's either true or false
...
3. **Challenge each criterion.** Ask: "If this passed but users were still unhappy, what's missing?" That missing thing is another criterion.
